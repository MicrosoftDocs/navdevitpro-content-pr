...
```
GET /businesscentral/companies({id})/salesCreditMemos({id})/salesCreditMemoLines
```
...
```json
GET https://api.businesscentral.dynamics.com/v1.0/api/beta/companies({id})/salesCreditMemos({id})/salesCreditMemoLines
```
