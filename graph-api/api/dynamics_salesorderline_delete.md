...
## HTTP request
```
DELETE /businesscentral/companies({id})/salesOrders({id})/salesOrderLines
```
...
```json
DELETE https://api.businesscentral.dynamics.com/v1.0/api/beta/companies({id})/salesOrders({id})/salesOrderLines
```
