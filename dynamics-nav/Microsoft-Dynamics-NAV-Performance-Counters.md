...
The following table describes the performance counters that are available in [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] for monitoring [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instances.  
  
|Category|Counter|[!INCLUDE[bp_tabledescription](../dynamics-nav/includes/bp_tabledescription_md.md)]|  
|--------------|-------------|---------------------------------------|  
|Client, NAS, and Web Service sessions|\# Active sessions|Number of active sessions on the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance.<br /><br /> An active session is a connection to the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance from a [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] client, such as the [!INCLUDE[nav_windows](../dynamics-nav/includes/nav_windows_md.md)] or [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)], NAS, or Web services.|  
||Server operations\/sec|Number of operations that have started on the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] per second.<br /><br /> An operation is a call to the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance from a [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] client to run [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] objects. **Note:**  OData and SOAP requests are not included.|  
||Average server operation time \(ms\)|Average duration of server operations in milliseconds.|  
|SQL Server connections|\# Mounted tenants|Number of tenants that are mounted on the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance.<br /><br /> For more information about tenants, see [Multitenant Deployment Architecture](../dynamics-nav/Multitenant-Deployment-Architecture.md).|  
||\# Open connections|Count of the current number of open connections from the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance to [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] databases on SQL Servers.|  
...
||\# Preferred connection total requests|Count of the total number of requests to the preferred connection cache. The preferred connection cache contains requests from the SQL connection pool that was last used by a [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] user.|  
...
|Data and caching|\# Calculated fields cache total requests|Count of the total number of requests to the calculated fields cache. The calculated fields cache contains the results of [CALCFIELDS Function \(Record\)](../dynamics-nav/CALCFIELDS-Function--Record-.md) calls.|  
