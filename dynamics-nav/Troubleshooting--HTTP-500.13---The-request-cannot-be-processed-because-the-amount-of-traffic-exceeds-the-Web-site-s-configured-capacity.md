...
When you try to preview a report or open a report as a PDF from the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)], you get an error message that is similar the following:  
...
## Resolution  
 ...
 To resolve this error, change the web.config file of the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)] to increase the limit on concurrent report sessions.  
  
#### To change the maximum concurrent report sessions in the web.config file  
  
1.  In a text editor, such as Notepad, open the web.config file for the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)].  
  
     The web.config file is located on the computer that is running the [!INCLUDE[nav_web_server](../dynamics-nav/includes/nav_web_server_md.md)]. The default folder is [!INCLUDE[navnow_install](../dynamics-nav/includes/navnow_install_md.md)]\\Web Client.  
...
2.  In the `<appsettings>` element, add the following element.  
    ...
    > [!IMPORTANT]  
    >  The number of report sessions that are running at the same time can affect the performance and reliability of the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)]. We recommend that you increase the value slightly, for example from 50 to 75, and then evaluate the performance and increase or decrease as necessary.  
