...
| `<RecreateScript>`| Used for reloading the control add-in script in following instances:<ul><li>In a FastTab, when the user selects **Show More** or **Show Less**.</li><li>In Designer, when the user switches the display target.</li></ul>If this element is not defined, then the `<Script>` elememt is used. This element only pertains to the [!INCLUDE[nav_windows_md](includes/nav_windows_md.md)].|
