...
>   
>  Phantom reads cannot occur with SERIALIZABLE locking because the filter range is locked instead of the records. Phantom reads can occur with REPEATABLE READ locking because only the records are locked. You should know this difference when you upgrade from an earlier version to [!INCLUDE[navnowlong](includes/navnowlong_md.md)]. For more information about phantom reads, see [Concurrency Effects](https://go.microsoft.com/fwlink/?LinkId=251936) in the MSDN Library.  
