...
If a [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance is configured for multitenancy, then you can mount and dismount tenant databases by using the [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)] or **Mount\-NAVTenant** or **Dismount\-NAVTenant** cmdlets in [!INCLUDE[nav_shell](../dynamics-nav/includes/nav_shell_md.md)].  
  
 Mounting a tenant by using the [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)] requires that you have a running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance. This is not required when you use the [!INCLUDE[nav_shell](../dynamics-nav/includes/nav_shell_md.md)]. With the **Mount\-NAVTenant**, you can mount a tenant by either specifying the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance, if it is running, or the application database.  
  
 This topic describes how to use the [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)] to mount or dismount a tenant. For more information about how to use the [!INCLUDE[nav_shell](../dynamics-nav/includes/nav_shell_md.md)], see [T:Microsoft.Dynamics.Nav.Management.Cmdlets.Mount\-NAVTenant](assetId:///T:Microsoft.Dynamics.Nav.Management.Cmdlets.Mount-NAVTenant) and [T:Microsoft.Dynamics.Nav.Management.Cmdlets.Dismount\-NAVTenant](assetId:///T:Microsoft.Dynamics.Nav.Management.Cmdlets.Dismount-NAVTenant).  
...
2.  Verify that the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] that you want to mount the tenant on is running, and if not, then start it:  
  
    1.  In the left pane of [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)], choose a [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] computer. Unless you are administering a remote computer, this is **Microsoft Dynamics NAV \(local\)**.  
  
    2.  In the center pane, if the **Status** of the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance is **Stopped**, then right\-click it, and then choose **Start**.  
  
3.  In the left pane, expand item for the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance on which you want to mount the tenant, and then choose the **Tenants** item.  
...
4.  To mount a tenant, in the **Actions** pane, choose **Mount Tenant**, fill in the parameters on the **Mount Tenant** dialog box, and then choose the **OK** button.  
    ...
    |Setting|[!INCLUDE[bp_tabledescription](../dynamics-nav/includes/bp_tabledescription_md.md)]|Required|  
    |-------------|---------------------------------------|--------------|  
    ...
    |Database Name|Specifies the name of the [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] database that you want to mount on the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance. For example, to mount the [!INCLUDE[demolong](../dynamics-nav/includes/demolong_md.md)], specify **Demo Database NAV \(9\-0\)**.|Yes|  
    |Database Server|Specifies the network name of the computer that is running SQL Server and the SQL Server database instance to connect to. You specify the value by using the format ComputerName\\SQLServerInstance. The default database instance for [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] is NAVDEMO.|No|  
    |Alternate Tenant IDs|Specifies the alternative IDs for the tenant, such as host names for the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)], SOAP web services, OData web services, or the [!INCLUDE[nav_windows](../dynamics-nav/includes/nav_windows_md.md)].<br /><br /> If you use alternative IDs for tenant resolution in the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)], you must also enable some of the UrlRewrite rules in the web.config file for the [!INCLUDE[nav_web_server](../dynamics-nav/includes/nav_web_server_md.md)].|No|  
    ...
    |Enable NAS Services|Specifies whether NAV Application Server services are enabled for the tenant. For more information about NAS, see [Configuring NAS Services](../dynamics-nav/Configuring-NAS-Services.md).|No|  
    |Run NAS Services with Admin Rights|Specifies whether NAS services run operations with administrator rights instead of the rights that are granted to the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] service account.<br /><br /> -   If you select this setting, NAS services will have full permissions in [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)], similar to the permissions that are granted by the SUPER permission set. The [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] service account is not required to be set up as a user in [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)].<br />-   If you clear this setting, the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] service account must be added as a user in [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] and assigned the permissions that are required to perform the operations.|No|  
    |Default Company|Specifies the default [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] company for the tenant. A tenant can contain several companies. This is the company that opens by default in [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] client.|No|  
...
5.  To save the changes, choose the **OK** button.  
     ...
     Encryption keys are to secure the login credentials over the connection between the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance and the [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] database in SQL Server.  
