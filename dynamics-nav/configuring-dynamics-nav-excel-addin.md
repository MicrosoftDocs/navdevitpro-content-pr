...
You can set up the [!INCLUDE[navnow](includes/navnow_md.md)] deployment to support an Excel addin that enables users in the [!INCLUDE[navnow](includes/navnow_md.md)] client to work with data in list pages in Excel. Users can get fresh data from [!INCLUDE[navnow](includes/navnow_md.md)] and update the data in [!INCLUDE[navnow](includes/navnow_md.md)] based on their work in Excel.

Without the add-in, users can only open a list page in Excel from the **Open in Excel** action on the page. When the add-in is set up, the **Open in Excel** action is replaced by the **Edit in Excel** action.
...
You can add the Azure AD application by using the Azure portal at [http://manage.windowsazure.com](http://manage.windowsazure.com). 
...
1.  Add an Azure AD application for the Excel add-in.
    ...
    For more information about how to add an application to an Azure AD tenant, see [Register your application with your Azure Active Directory tenant](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-app-registration).
...
3.  Configure OAuth2 authentication in the Excel add-in application manifest.
    ...
    The Excel add-in requires OAuth2 implicit grant flow to be enabled on the application. The manifest for the application is a .json file type. To enable OAuth2 implicit grant flow in th manifest file, change the *oauth2AllowImplicitFlow* key to *true*. 
