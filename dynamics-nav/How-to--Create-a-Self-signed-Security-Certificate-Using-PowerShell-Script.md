...
With the `New-NavSelfSignedCertificate` PowerShell cmdlet, you can create a self\-signed certificate to help secure the communication between the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] and the [!INCLUDE[nav_web](../dynamics-nav/includes/nav_web_md.md)].  
...
1.  On the computer or virtual machine that is running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)], run Microsoft Dynamics NAV Administration Shell as an Administrator.  
...
5.  Next, at the command prompt, type the following command: `New-NavSelfSignedCertificate <parameters>`  
    ...
    |`-TestCertificateServerAddress`|Type:String|The authority\/entity that this certificate is issued for. Default is the computer host name. Specify this value if the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] is accessed by using an address that differs from the computer name. This parameter is optional.|  
    ...
    |`-NavServiceIdentity`|Type:String|Username for the identity running the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)]. Default is `NTAUTHORITY\Network Service`. This parameter is optional.|  
