...
## HTTP request
```
POST /businesscentral/companies({companyId})/vendors({vendorId})/defaultDimensions
```
...
```json
POST https://api.businesscentral.dynamics.com/v1.0/api/beta/companies({companyId})/vendors({vendorId})/defaultDimensions
```
