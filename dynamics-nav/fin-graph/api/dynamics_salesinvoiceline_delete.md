...
```
DELETE businesscentralPrefix/companies({id})/salesInvoices({id})/salesInvoiceLines(documentId=({id}),sequence=({number}))
```
...
```json
DELETE https://{businesscentralPrefix}/api/beta/companies({id})/salesInvoices({id})/salesInvoiceLines(documentId=({id}),sequence=({number}))
```
