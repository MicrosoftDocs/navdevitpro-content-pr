...
```
PATCH businesscentralPrefix/companies({id})/salesCreditMemos({id})/salesCreditMemoLines(documentId=({id}),sequence=({number}))
```
...
```json
PATCH https://{businesscentralPrefix}/api/beta/companies({id})/salesCreditMemos({id})/salesCreditMemoLines(documentId=({id}),sequence=({number}))
Content-type: application/json
