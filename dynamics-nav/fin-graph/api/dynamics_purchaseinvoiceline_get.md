...
```
GET /businesscentral/companies({id})/purchaseInvoices({invoiceid})/purchaseInvoiceLines(documentId=({id}),sequence=({number}))
```
...
```json
GET https://api.businesscentral.dynamics.com/v1.0/api/beta/companies({id})/purchaseInvoices({invoiceid})/purchaseInvoiceLines(documentId=({id}),sequence=({number}))
```
