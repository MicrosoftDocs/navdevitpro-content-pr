...
Secure Sockets Layer \(SSL\) is a cryptographic protocol that helps provide security and data integrity for data communications over a network. By encrypting your [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] web services using SSL, you make your data and the network more secure and more reliable.  
  
## Microsoft Dynamics NAV Web Services and SSL  
 [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] supports SSL authentication for SOAP and OData web services. The server authenticates itself to the client, but the client does not authenticate itself to the server. When the web service client connects to the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance, the server replies by sending its digital certificate to the client. This certificate contains the server's public encryption key and the name of the authority that granted the certificate. The client verifies the certificate using the authority's public key.  
...
## Story  
 Victor, who is a business systems developer at [!INCLUDE[demoname](../dynamics-nav/includes/demoname_md.md)], knows that his implementation of web services applications for [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] is unlikely to pass a company security audit unless he encrypts sensitive data that is transmitted over the company intranet. He decides to protect web services communication with SSL.  
  
##  <a name="ConfigureSSL"></a> Configuring Microsoft Dynamics NAV Web Services to Use SSL  
 The first step is to prepare [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] to use SSL. This involves configuring the relevant [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance to specify SSL.  
  
 You can configure [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instances with the [Microsoft Dynamics NAV Server Administration Tool](../dynamics-nav/Microsoft-Dynamics-NAV-Server-Administration-Tool.md) or [Microsoft Dynamics NAV Windows PowerShell Cmdlets](../dynamics-nav/Microsoft-Dynamics-NAV-Windows-PowerShell-Cmdlets.md). For more information, see [Managing Microsoft Dynamics NAV Server Instances](../dynamics-nav/Managing-Microsoft-Dynamics-NAV-Server-Instances.md).  
  
 The following procedure uses [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)] to configure the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance.  
...
2.  In [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)], in the left pane, under **Console Root**, expand the node for the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] computer. This is typically named **Microsoft Dynamics NAV \(Local\)**, which is the local computer.  
  
     The [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instances on the computer appear in the left pane and center panes.  
...
1.  On the computer that is installed with Microsoft Visual Studio 2012 or Windows SDK, create a folder called *My Certificates* for working with and storing certificates.  
     ...
     You can work on the same computer that is running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] or a different computer.  
...
4.  To create the SSL certificate with a private key file, type the following command.  
    ...
    > [!TIP]  
    >  If you are working on the computer running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)], you can use the following command to create and install the certificate, so you can skip the next procedure for importing the certificate.  
    >   
...
6.  If you are not working on the computer running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)], then copy the .pfx to the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] computer.  
...
##  <a name="Importing"></a> Importing the SSL Certificate into the Local Computer Store of the Microsoft Dynamics NAV Server computer  
 Once you obtain a certificate, you must import it into the local computer store on the computer running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)]. The certificate file will have the extension .cer or .pfx.  
  
#### To import an SSL certificate into the local computer store  
  
1.  On the computer running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)], choose **Start**, and then choose **Run**.  
...
##  <a name="GrantingAccess"></a> Granting Permissions to the Certificate's Private Key to Microsoft Dynamics NAV Server  
 If the certificate has a private key, then you must give the service account of [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] access to the private key.  
...
4.  In the **Select Users, Computers, Service Accounts, or Groups** dialog box, in the **Enter the object names to select** box, type the name of the service account for [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)], and then choose **OK**.  
...
1.  On the computer running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)], open a command prompt as an administrator as follows:  
...
3.  To view the ACL for the web services ports to determine if any entries are already using the relevant host name and port, type the following command:  
    ...
    ```  
     ...
     By default, SOAP and OData web services in [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] use ports 7047 and 7048, respectively, and connect to a [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance named[!INCLUDE[nav_server_instance](../dynamics-nav/includes/nav_server_instance_md.md)]. The default reserved URL entries for these services are: `http://+:7047/nav_server_instance/` and `http://+:7048/nav_server_instance/`.  
...
4.  You must delete any entries that use the same port as the SOAP or OData web services.  
    ...
    ```  
     ...
     Substitute `Port` with the port number of the SOAP or OData web service and `NAVserver` with the name of the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance.  
...
5.  To register the ports for the SOAP and OData web service with https, type the following command for each service:  
    ...
    |Option|Description|  
    |------------|-----------------|  
    |`hostname`|The name of the computer running [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)]. Use `+` for localhost.|  
    |`port`|The port that is used by the web service.|  
    |`navserver`|The name of the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance to use with the web service. The default is [!INCLUDE[nav_server_instance](../dynamics-nav/includes/nav_server_instance_md.md)].|  
    |`DOMAIN\username`|The domain and user name of the service account for [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)]. If the service account for [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] is Network Service, then use "NT AUTHORITY\\NETWORKSERVICE".|  
  
     For example, if the service account for [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] has the domain *ABC* and the user name *xyz*, and then the command for the SOAP web service is as follows:  
  
    ```  
    netsh http add urlacl url=https://+:7047/nav_server_instance user="NT AUTHORITY\NETWORKSERVICE"  
    ```  
  
     If the service account for [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] is Network Service, then the command is as follows:  
...
3.  To bind an SSL certificate to the SOAP or OData web service port, use the following command:  
    ...
    |`id`|A 32\-digit hexadecimal number that identifies the [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] application. The id must be in the format {NNNNNNNN\-NNNN\-NNNN\-NNNN\-NNNNNNNNNNNN\). If you do not know the appid, then use any value, such as {00112233\-4455\-6677\-8899\-AABBCCDDEEFF}.|  
...
##  <a name="Restart"></a> Restart the Microsoft Dynamics NAV Server Instance  
 You can use the Microsoft Dynamics NAV Server Administration Tool to restart the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance.  
  
#### To restart [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)]  
  
1.  In [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)], in the left pane, under **Console Root**, choose the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] computer.  
  
2.  In the center pane, choose the [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)] instance that you have configured, and then in the right pane under **Actions**, choose **Restart**.  
  
3.  After the instance restarts, close [!INCLUDE[nav_admin](../dynamics-nav/includes/nav_admin_md.md)].  
