...
```
GET businesscentralPrefix/companies({id})/customerSales({id})
```
...
## Response
If successful, this method returns a ```200 OK``` response code and an **customerSales** object in the response body.
...
```json
GET https://{businesscentralPrefix}/api/v2.0/companies({id})/customerSales({id})
```
