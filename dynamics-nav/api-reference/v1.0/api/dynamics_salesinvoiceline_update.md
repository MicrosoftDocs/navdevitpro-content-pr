...
```
PATCH businesscentralPrefix/companies({id})/salesInvoices({id})/salesInvoiceLines({salesOrderLineId})
```
...
```json
PATCH https://{businesscentralPrefix}/api/v1.0/companies({id})/salesInvoices{id}/salesInvoiceLines({salesOrderLineId})
Content-type: application/json
