...
## HTTP request
```
PATCH /businesscentral/companies({companyId})/items({itemId})/defaultDimensions({itemId},{dimensionId})
PATCH /businesscentral/companies({companyId})/customers({customerId})/defaultDimensions({customer},{dimensionId})
PATCH /businesscentral/companies({companyId})/employees({employeeId})/defaultDimensions({employeeId},{dimensionId})
...
```json
PATCH https://api.businesscentral.dynamics.com/v1.0/api/v1.0/companies({companyId})/items({itemId})/defaultDimensions({itemId},{dimensionId})
```
