...
## Overview
-  There are two event trace providers that publish different trace events to the event log: **icrosoft-DynamicsNAV-Server** and **Microsoft-DynamicsNAV-Common**. The trace events in this article are grouped according to their event trace provider. You typically need to provide this name in the monitoring tool that you are using.  
...
|tag|Specifies the ID of the telemtry trace event. For system telemetry trace events, this ID is autogenerated. For custom telemetry trace events, it is user-defined.|Telemetry (TelemetryData)|  
