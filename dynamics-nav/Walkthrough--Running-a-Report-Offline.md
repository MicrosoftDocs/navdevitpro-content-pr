...
You can view a report and view the dataset and layout on a computer that does not have [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] installed. To do this, you must have Microsoft Visual Studio installed and you must export the report dataset from [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] as an .xml file and export the report layout .rdlc file from Visual Studio Report Designer.  
...
1.  On the computer with [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] installed, open the [!INCLUDE[nav_windows](../dynamics-nav/includes/nav_windows_md.md)].  
  
2.  In the **Search** box, enter **Customer – Top 10 List**, and then choose the related link.  
  
3.  On the request page for the Customer – Top 10 List report, on the **Application** menu ![Application Menu button in menu bar](../dynamics-nav/media/ApplicationMenuIcon.png "ApplicationMenuIcon"), choose **Help**, and then choose **About This Page** to activate the About This Report feature. For more information, see [How to: View and Export the Dataset for a Report](../Topic/How%20to:%20View%20and%20Export%20the%20Dataset%20for%20a%20Report.md).  
...
6.  On the report preview, on the Application menu ![Application Menu button in menu bar](../dynamics-nav/media/ApplicationMenuIcon.png "ApplicationMenuIcon"), choose **Help**, and then choose **About This Report** to display the report dataset.  
  
7.  On the **About This Report** page, choose **Export as XML**.  
  
8.  In the **Export File** dialog box, choose **Save**.  
  
     ![Export File dialog box. Choose the Save button.](../dynamics-nav/media/Plat_ExportFile.png "Plat\_ExportFile")  
...
4.  In Visual Studio, in **Solution Explorer**, select **Report.rdlc**, and then in the **Properties** window, note the path where the Report.rdlc file is located.  
     ...
     ![Visual Studio 2010 Solution Explorer, Properties](../dynamics-nav/media/Plat_ExportLayout_SolnExpl.png "Plat\_ExportLayout\_SolnExpl")  
...
> [!NOTE]  
>  If the report references external assemblies, then you must install the assemblies in the global assembly cache on the computer on which you want to run the report. For more information, see [EnableExternalAssemblies Property](../dynamics-nav/EnableExternalAssemblies-Property.md) and [Extending Microsoft Dynamics NAV Using Microsoft .NET Framework Interoperability](../dynamics-nav/Extending-Microsoft-Dynamics-NAV-Using-Microsoft-.NET-Framework-Interoperability.md),  
  
#### To run the report by using Visual Studio  
  
1.  Copy the Dataset.xml file and Report.rdlc file from the first computer, which has [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] installed, to your desktop on the second computer, which does not have [!INCLUDE[navnow](../dynamics-nav/includes/navnow_md.md)] installed.  
...
4.  In the **New Project** window, under **Installed Templates**, expand **Visual C\#**, choose **Windows**, and then choose **Windows Forms Application**.  
     ...
     ![Visual Studio 2010, New Visual C&#35; Project](../dynamics-nav/media/Plat_ExportReport_.png "Plat\_ExportReport\_")  
...
6.  In Solution Explorer, right\-click the OfflineReport project, choose **Add**, and then choose **Existing Item**.  
     ...
     ![Visual Studio 2010, Add Existing to project](../dynamics-nav/media/Plat_ExportReport_AddExisting.png "Plat\_ExportReport\_AddExisting")  
...
7.  In the **Add Existing Item** dialog box, navigate to your Desktop, set the file type to **All Files \(\*.\*\)**, select the Dataset.xml file and the Report.rdlc file, and then choose **Add**.  
     ...
     ![Add Existing Item dialog box, file type All Files](../dynamics-nav/media/Plat_ExportReport_AddFiles.png "Plat\_ExportReport\_AddFiles")  
...
9. In the **Toolbox** window, under **Reporting**, drag a new **Report Viewer** control to Form1.  
     ...
     ![Visual Studio 2010, Report Viewer control](../dynamics-nav/media/Plat_ExportReport_ReportViewerCtrl.png "Plat\_ExportReport\_ReportViewerCtrl")  
...
10. Select the report viewer control, and then in the **Properties** window, under **Misc**, expand **Local Report**, and then set the **ReportEmbeddedResource** property, to **OfflineReport.Report.rdlc**.  
     ...
     ![Visual Studio, ReportEmbeddedResource property](../dynamics-nav/media/Plat_ExportReport_ReportEmbeddedResource.png "Plat\_ExportReport\_ReportEmbeddedResource")  
...
11. In Solution Explorer, select the OfflineReport project, and on the shortcut menu, choose **Add**, and then choose **Class**.  
     ...
     ![Visual Studio 2010, Add a Class to a solution](../dynamics-nav/media/Plat_ExportReport_AddClass.png "Plat\_ExportReport\_AddClass")  
  
12. In the **Add New Item** window, under **Installed Templates**, choose **Visual C\# Items**, and then choose **Class**.  
  
     ![Visual Studio, Add a Class, select Visual C&#35; class](../dynamics-nav/media/Plat_ExportReport_SelectClass.png "Plat\_ExportReport\_SelectClass")  
...
19. On the **Debug** menu, choose **Start Debugging** to view the report in the report viewer control.  
     ...
     ![Visual Studio 2010, Debug menu, Start Debugging](../dynamics-nav/media/Plat_ExportReport_Report.png "Plat\_ExportReport\_Report")  
