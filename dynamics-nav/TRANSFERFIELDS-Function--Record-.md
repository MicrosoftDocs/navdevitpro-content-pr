...
## Remarks  
 **TRANSFERFIELDS** copies fields based on the [Field No. Property](../dynamics-nav/Field-No.-Property.md) of the fields. For each field in *Record* \(the destination\), the contents of the field that has the same Field No. in *FromRecord* \(the source\) will be copied, if such a field exists.  
