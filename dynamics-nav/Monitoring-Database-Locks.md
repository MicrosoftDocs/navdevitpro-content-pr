...
## About Locks
Microsoft SQL Server uses locking to control access by mutiple users to the same data at the same time. To protect a transaction against other transactions modifying the same data, the first transaction puts a lock on the data until the it is done.
...
|SQL Lock Resource Type|Specifies the database resource affected by the lock, such as|  
...
|AL ScopeName|Specifies the C/AL function that ran the transaction that caused the lock.|
