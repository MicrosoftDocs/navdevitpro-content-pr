...
Executed when a control add\-in on the [!INCLUDE[nav_windows](../dynamics-nav/includes/nav_windows_md.md)] page sends event information to the server\-side business logic. You add C\/AL code to process the data that is sent with the event.  
...
> [!NOTE]  
>  The data type is determined by the binding target that is specified in the [SourceExpr Property](../dynamics-nav/SourceExpr-Property.md) of the page field control. If the binding target is of type BigText, then the Data parameter is BigText. Otherwise, the Data parameter is Text  
...
## Remarks  
 An example of an event is when a user chooses a button in the control add\-in user interface or when a user changes a value in a field. A control add\-in can use events like these to invoke the OnControlAddin trigger and pass information as an integer in the *Index* parameter and as a text string in the *Data* parameter. To process the information, you add the C\/AL code to the OnControlAddin trigger. For more information, see [Exposing Events and Calling Respective C\-AL Triggers from a Windows Client Control Add\-in](../dynamics-nav/Exposing-Events-and-Calling-Respective-C-AL-Triggers-from-a-Windows-Client-Control-Add-in.md).  
