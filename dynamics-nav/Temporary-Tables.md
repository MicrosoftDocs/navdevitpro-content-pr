...
## Advantage of a Temporary Table  
 The advantage of using a temporary table is that all the interaction with a temporary table occurs on [!INCLUDE[nav_server](../dynamics-nav/includes/nav_server_md.md)]. This reduces the load on both the network and the SQL database server.  
...
> [!NOTE]  
>  XMLPort and report objects include a Temporary property that enables you to use temporary tables for storing the elements of data set that is generated by the objects. For more information, see [Temporary Property \(XMLports\)](../dynamics-nav/Temporary-Property--XMLports-.md) and [Temporary Property \(Reports\)](../dynamics-nav/Temporary-Property--Reports-.md).  
...
2.  Choose **View**, and then choose **C\/AL Globals** or **C\/AL Locals**, depending on whether your variable will be global or local.  
     ...
     ![C&#47;AL Globals window](../dynamics-nav/media/NAV_ADG_8_Table_31.gif "NAV\_ADG\_8\_Table\_31")  
...
4.  With the cursor still on the line that defines the temporary table, choose **View**, and then choose **Properties** \(Shift\+F4\) to display the **Properties** window.  
     ...
     ![Properties window](../dynamics-nav/media/NAV_ADG_8_Table_32.gif "NAV\_ADG\_8\_Table\_32")  
