...
||SystemPartID|Partially supported.|[!INCLUDE[nav_web](includes/nav_web_md.md)] supports Notes and RecordLinks system parts, but not Outlook and MyNotes system parts. RecordLinks only support access to URLs and resources on file shares; files that are stored locally on the user's computer cannot be accessed. |  
